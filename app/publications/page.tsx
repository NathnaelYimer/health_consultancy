  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 lg:px-8 py-16">
        <h1 className="text-3xl lg:text-5xl font-bold mb-6">Our Publications</h1>

        <p className="text-muted-foreground mb-8 max-w-3xl">
          Sharing evidence to inform health decisions, policy, and practice. Download articles, reports and policy
          briefs below.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {publications.map((p) => (
            <Card key={p.id}>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground mb-4">{p.authors} — {p.year}</p>
                <div className="flex gap-4">
                  <Link href={`/publications/${p.id}`} className="underline">Read Summary</Link>
                  <Link href={p.pdf ?? '#'} className="underline">Download PDF</Link>
                </div>
              </CardContent>
            </Card>
          ))}
